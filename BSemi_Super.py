...
def semi_supervised(args):
    ...
    # define directory to save weight & biases
    wandb_log_path = save_dir / 'experiments' / f'exp_{args.exp}_opt_{args.opt}'
    os.makedirs(wandb_log_path, exist_ok=True)
    
    # initializing config of wandb with orginal hyperparameters
    wandb.init(project="BSemi_Super_res50",config=args,dir=wandb_log_path)
    config = wandb.config
    ...
    with open(save_dir /'params'/'pretrain'/args.ds_pr/f'{args.exp}_{args.model_type}param.json') as file:    
        HPP = json.load(file)
        in_size = HPP['in_size']
        print(f'\n size of images in pretrained model:{in_size}')
    ...
    for train_split in config['exp_split']:
        ...
        if config['fine']:
            ...
            for i in range(config['burn_in'],n_ckts_tot):
                ...
                if args.model_depth == 'res18':
                    ...
                    fv_size = 512    # the size of embedding vector in resnet18 before avg layer
                ...
                print(f'Best model from epoch {best_epoch} for inference on {i}th cehkpoint was loaded!')
        ...
        # whenever we want to save a total pr when we are not in weight & biases
        if config['pr_ens']:
